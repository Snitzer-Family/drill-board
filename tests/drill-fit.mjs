...
check("portrait half-ice (attack=down)", () => {
  ...
  const n = toPx(183, 42.5), mn = map(n.x, n.y);
  near(mn.x, 183, 0.5, "net x"); near(mn.y, 42.5, 0.5, "net y");
});
...
check("full ice paired goal/blue lines", () => {
  ...
  const lm = [
    { feature: "goal_line", ...px(17, 42.5) },
    { feature: "goal_line", ...px(183, 42.5) },
...
check("transformDsl rewrite", () => {
  ...
  assert.equal(out[2], "PIECE N1 net 183 42.5 goalie", "net snapped to goal line");
