...
// favicon at all. These carry the SMALL rendition — a PNG cannot evaluate the
// size media query, so the plan would be the wrong picture at 16px.
...
const small = appIconSvg("small");

// The rasters are bitmaps; nothing about them tells a test whether they still
// match the art. Stamping each source variant's hash into icon.svg gives the
// drift guard something to compare, so changing ONLY the raster-side geometry
// still fails the suite instead of shipping stale bitmaps. Both are stamped
// because the two rasterised variants are now different pictures.
const sha = s => createHash("sha256").update(s).digest("hex");
writeFileSync(join(PUBLIC, "icon.svg"),
  `<!-- generated by scripts/build-icons.mjs — do not edit;`
  + ` plan sha256:${sha(bleed)} crease sha256:${sha(small)} -->\n${tab}`);
...
for (const px of ICO_SIZES) {
  ...
  await shoot(small, f, px, { alpha: true });
