  ...
  pieces.forEach(p => {
    ...
    const rnd = lm && lm !== "reactive"
      ? (lm === "always"
          ...
          : ` mode=${lm}`)
      : "";
    out.push(`PIECE ${p.id} ${p.kind} ${f1(p.x)} ${f1(p.y)} ${p.color}${lbl}${hnd}${sm}${car}${gp}${pas}${terms}${hld}${wt}${fac}${gl}${crs}${df}${lck}${siz}${grp}${lgt}${lin}${lgap}${lq}${lnx}${lfeed}${lconn}${cue}${rnd}${spd}`);
