...
Transcription rules:
- Use PIECE and PATH statements. SKATING IS CURVED: transcribe routes with Q curve segments that follow the drawn line's bend — skaters carve arcs, and drills draw them that way. Use L only where the drawing shows a genuinely straight, ruler-like segment; anything that turns, curls, buttonhooks, or loops is a curve — NEVER a chain of straight lines with corners. Capture the drill's shape and flow, not every wiggle.
...
- A dashed or hooked line that ends at a FREE-STANDING puck (often at the boards) rather than at a player is a CHIP into space: the drawn free puck marks where the chip LANDS — do not transcribe that puck as a piece. Write the chipper's puck with the chip handoff (chip=<pt>:<collector>@<recv>) and route the collector THROUGH the drawn puck spot, so their collect waypoint sits exactly on it. Never rewrite a chip as a plain pass, and never have the collector receive at the chipper's feet — they collect where the free puck is drawn. A puck drawn at the wall sits ON THE ICE a stride or two off the boards — read its true pixel position inside the board line (and report end_boards so that strip scales). After a boards collect the player carries a few feet INTO the ice before any release: put the release waypoint where the dashed line actually leaves the route in the photo — never at the collect point or on the boards.
...
- IGNORE ALL LOGOS. Brand logos, watermarks (IHS etc.), page borders, bench areas outside the boards, and the rink outline itself are never drill content — transcribe nothing from them, no matter how line-like they look.
- A shaded/highlighted area or any freehand drawing ON the ice is a coaching overlay: trace it faithfully with a MARK statement in a matching colour, and when the area is shaded/tinted, give it the matching translucent fill (e.g. MARK Z1 #e8c547 0.6 dashed fill=e8c547:0.25 x1,y1 x2,y1 x2,y2 x1,y2 x1,y1 — pixel coords like everything else). For straight-sided shapes give the corner points (the app keeps straight sides straight); for curved traces give points along the curve. Shaded GEOMETRIC shapes (a highlighted faceoff circle, a rectangle, a triangle zone) are the app's preset shape markers — trace the IDEAL outline, not a wobbly freehand: a circle/oval gets 12–16 evenly spaced points ON the drawn outline (the app snaps a clean round trace to a perfect circle), a rectangle/triangle gets exactly its corner points, and the shading always becomes fill=. MARK is ONLY for shaded zones/overlay drawings — NEVER for arrows, passes, shots, or player movement; any arrow in the diagram must become drill logic (a PATH, pass=, or shoot=), never ink. Small black rings at a zone's corners are tires, NOT cones — cones are only open triangles/wedges.
