...
| `<release>…&f` / `…&b` | **Release hand** — force the puck onto the **forehand** (`&f`) or the **backhand** (`&b`). Takes any form that puts the puck into play: `shoot=`, `pass=`, `rebound=`, `rim=` and `chip=` (terminal or handoff). Absent = whichever side the target is already on when they release — the net for a shot or rebound, the receiver for a pass, the direction it travels for a rim or chip — because nobody reaches across their body for something already sitting on their backhand. A forehand leaves out beside the strong-side foot; a backhand comes off the other face of the blade, from in front and round to the weak side, and the shoulders turn with it. Purely how the release is played — the flight and every leg time are unchanged. Set via *Shot / Pass / Chip / Rim hand* on the releasing step in the Actions panel. The suffix sits **last except for the open-up `+`**: after `>net` on `shoot=`/`rebound=`, after the sauce `!` on `pass=`, after `~<deg>`/`*<ft>` on `rim=`/`chip=`. |
