...
| `sym=<text>` | player | Whiteboard-mode symbol (≤3 chars, e.g. `X`, `O`, `F`, `LW`, `RD`; underscores read as spaces). `△`, `○`, `□` render as drawn shapes rather than text. Shown instead of the skater when **Whiteboard mode** is on (Settings). Unset players default to `X` if `defense`, else `O`. |
...
| `group=<name>` | any (incl. label, MARK) | Named-group membership (underscores read as spaces). A plain grab moves just that piece; select the group (box-select, or ⌖ in the Visibility menu) to drag, rotate, or duplicate the whole formation. In the app: box-select → *◇ Group*, or the **Group** field on any piece popup; **☰ → Visibility…** lists every group with select and show/hide toggles, alongside show/hide for whole item categories (players, pucks, nets, routes, …) — hiding is a session-only view state, never written to the file and never a change to drill timing. |
